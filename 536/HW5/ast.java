...
class ProgramNode extends ASTnode {
	...
	public Type typeCheck(TypeContext ctx) {
		return myDeclList.typeCheck(ctx);
	}
}
...
class FnBodyNode extends ASTnode {
	...
	public void typeCheck(TypeContext ctx) {
		myDeclList.typeCheck(ctx);
		myStmtList.typeCheck(ctx);
	}
...
class ExpListNode extends ASTnode {
    ...
    public List<Type> typeCheck(TypeContext ctx) {
    	List<Type> types = new ArrayList<>(myExps.size());
    	
    	for (ExpNode node : myExps) {
    		types.add(node.typeCheck(ctx));
    	}
...
abstract class DeclNode extends ASTnode {
	...
	public abstract Type typeCheck(TypeContext ctx);
}
...
class FnDeclNode extends DeclNode {
	...
	public Type typeCheck(TypeContext ctx) {
		FnSym prev = ctx.getFnSym();
		ctx.setFnSym((FnSym) myId.sym());
		myBody.typeCheck(ctx);
		ctx.setFnSym(prev);
...
class ReadStmtNode extends StmtNode {
	...
	public Type typeCheck(TypeContext ctx) {
		...
		} // Otherwise, type is Int / Bool / String / Void / Error
...
class WriteStmtNode extends StmtNode {
	...
	public Type typeCheck(TypeContext ctx) {
		...
		} // Otherwise, type is Int / Bool / String / Error
...
class IfElseStmtNode extends StmtNode {
	...
	public Type typeCheck(TypeContext ctx) {
		...
		myThenDeclList.typeCheck(ctx);
		myThenStmtList.typeCheck(ctx);
		myElseDeclList.typeCheck(ctx);
		myElseStmtList.typeCheck(ctx);
...
class ReturnStmtNode extends StmtNode {
	...
	public Type typeCheck(TypeContext ctx) {
		FnSym sym = ctx.getFnSym();
		if (sym == null) {
			System.err.println("Unexpected return statement node outside of function");
			System.exit(-1);
		}
		...
		if (myExp == null) {
			if (returnType.isBoolType() || returnType.isIntType()) {
...
class IdNode extends ExpNode {
	...
	public Type typeCheck(TypeContext ctx) {
		if (mySym == null) {
			ErrMsg.fatal(myLineNum, myCharNum, "Unexpected unlinked identifier node");
		}
...
class AssignNode extends ExpNode {
    ...
    public Type typeCheck(TypeContext ctx) {
		Type left = myLhs.typeCheck(ctx);
		Type right = myExp.typeCheck(ctx);

		if (left.isErrorType() || right.isErrorType()) {
			return new ErrorType();
		}
		
		if (left.isFnType() && right.isFnType()) {
			ErrMsg.fatal(myLhs.lineNum(), myLhs.charNum(), "Function assignment");
			return new ErrorType();
		} else if (left.isStructDefType() && right.isStructDefType()) {
			ErrMsg.fatal(myLhs.lineNum(), myLhs.charNum(), "Struct name assignment");
			return new ErrorType();
		} else if (left.isStructType() && right.isStructType()) {
			ErrMsg.fatal(myLhs.lineNum(), myLhs.charNum(), "Struct variable assignment");
			return new ErrorType();
		} else if (!((left.isBoolType() && right.isBoolType()) || (left.isIntType() && right.isIntType()))) {
...
class CallExpNode extends ExpNode {
	...
	public Type typeCheck(TypeContext ctx) {
		if (!myId.sym().getType().isFnType()) {
			...
			List<ExpNode> exps = myExpList.getExpNodes();
			
			if (sym.getNumParams() != exps.size()) {
				ErrMsg.fatal(myId.lineNum(), myId.charNum(), "Function call with wrong number of args");
				return new ErrorType();
			}
			
			List<Type> formalTypes = sym.getParamTypes();
			
			for (int i = 0; i < exps.size(); i += 1) {
				Type expType = exps.get(i).typeCheck(ctx);
				if (expType.isErrorType() || expType.equals(formalTypes.get(i))) {
					continue;
				} else {
...
class PlusNode extends BinaryExpNode {
	...
	@Override
	public Type typeCheck(TypeContext ctx) {
		Type type1 = myExp1.typeCheck(ctx);
		Type type2 = myExp2.typeCheck(ctx);
